...
contract VUSDMasterV2 is Ownable, Withdrawable, ReentrancyGuard, Pausable {
    ...
    IERC20 public xvon; 
    ISushiSwapRouter public immutable wswapRouter; // SushiRouter
    address public treasury; // feeCollector
    address public strategist; // strategy contract
    
    address[] public swapPath;
    address[] public swapPathReverse;
    
    uint public xvonPermille = 0; // no XVON collateral
    ...
    mapping(address => uint256) public vusdClaimAmount;
    mapping(address => uint256) public vusdClaimBlock;
    mapping(address => uint256) public busdClaimAmount;
    mapping(address => uint256) public busdClaimBlock;
    
    event Stake(address indexed user, uint256 amount);
    event VUSDClaim(address indexed user, uint256 amount);
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused onlyUser {
        ...
        vusdClaimAmount[msg.sender] = amount;
        vusdClaimBlock[msg.sender] = block.number;
    ...
    function claimVUSD() internal whenNotPaused onlyUser {
        ...
        uint256 amount = vusdClaimAmount[msg.sender];
        vusdClaimAmount[msg.sender] = 0;
        vusd.transfer(msg.sender, amount);
        
        emit VUSDClaim(msg.sender, amount);
    }
    ...
    function redeem(uint256 amount) internal whenNotPaused {
        ...
        vusd.transferFrom(msg.sender, dead, amount);
    ...
    function claimBUSD(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 busdTransferAmount = amount * (10000 - xvonPermille - treasuryPermille) / 10000; // 1000 * (1000 - 0 - 250) / 10000
        uint256 busdTreasuryAmount = amount * treasuryPermille / 10000;
        uint256 xvonTransferAmount = xvon.balanceOf(address(this)) * amount / vusd.totalSupply();
        vusd.burn(dead, amount);
    ...
    function emergencyRedeemAll() external nonReentrant whenPaused {
        uint256 amount = vusd.balanceOf(msg.sender);
        require(amount > 0, 'amount cant be zero');
        require(busdClaimAmount[msg.sender] == 0, 'you have to claim first');
        vusd.transferFrom(msg.sender, dead, amount);
        busdClaimAmount[msg.sender] = amount;
    ...
    function emergencyClaimBUSDAll() external nonReentrant whenPaused {
        ...
        uint256 busdTransferAmount = amount * (10000 - xvonPermille - treasuryPermille) / 10000;
        vusd.burn(dead, amount);
        busd.safeTransfer(msg.sender, busdTransferAmount);

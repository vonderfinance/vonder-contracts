...
contract Express is Ownable {
    ...
    event Withdraw(string, address, uint256);
    ...
    function adminEmergencyWithdrawXVON(uint256 _amount) external onlyOwner {
    ...
    function adminEmergencyWithdrawVON(uint256 _amount) external onlyOwner {
        require(_amount > 0, "Should not be zero");

        IERC20 _VON = IERC20(von);
        uint256 _balance = _VON.balanceOf(address(this));

        require(_balance > 0, "Nothing to withdraw");
        require(_amount <= _balance, "Exceed balance");

        _VON.transfer(msg.sender, _amount);
    }
    ...
    function burnVON(uint256 _amountBurn) external onlyAdmin {
        IERC20 _VON = IERC20(von);
        _VON.safeTransferFrom(msg.sender, burnTo, _amount);
    }
    ...
    function setVON(address _von) external onlyOwner {
        von = _von;
    }

    function setXVON(address _xvon) external onlyOwner {
        xvon = _xvon;
    }
